...
def cluster_display(g, labels, label_names, file_name):
    ...
    clusters = np.full(shape=(m,), fill_value="", dtype=np.chararray)
    for i in range(m):
...
def cluster_save(g, row_labels, col_label, file_name):
    ...
    g_df = pd.DataFrame.from_dict(data=g_dict, orient='index', columns=[col_label])
    print(g_df)
...
def color_clusters(h, k, codes):
    ...
    m = np.shape(h)[0]
    n = m + 1
    cluster_colors = np.full(shape=(2 * n * 1,), fill_value="", dtype=np.chararray)
